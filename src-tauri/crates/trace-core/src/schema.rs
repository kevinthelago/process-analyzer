...
// ── network ───────────────────────────────────────────────────────────────────
// IP addresses stored as FixedSizeBinary(16) — IPv6 bytes; IPv4 as IPv4-in-IPv6.

static NETWORK: LazyLock<SchemaRef> = LazyLock::new(|| {
    Arc::new(Schema::new(vec![
        Field::new("timestamp_ns",  DataType::Int64,               false),
        Field::new("process_id",    DataType::UInt32,              false),
        Field::new("thread_id",     DataType::UInt32,              true),
        ...
        Field::new("fd",            DataType::Int32,               true),
        Field::new("local_addr",    DataType::FixedSizeBinary(16), true),
        Field::new("remote_addr",   DataType::FixedSizeBinary(16), true),
        Field::new("local_port",    DataType::UInt16,              true),
        Field::new("remote_port",   DataType::UInt16,              true),
        Field::new("size_bytes",    DataType::UInt64,              true),
        Field::new("duration_ns",   DataType::UInt64,              true),
    ]))
});

pub fn network_schema() -> SchemaRef { NETWORK.clone() }
...
// ── stacks ────────────────────────────────────────────────────────────────────
// Exploded call stacks: (stack_id, depth, frame_id) triples.
// cpu_samples.stack_id FK → stacks.stack_id; depth 0 = leaf/top.
